...
/// 표시 시각은 capture_time(찍힌 시점)만 쓴다 — received_at(업로드) 폴백 없음
/// ([photoWatermarkTime]). 시각이 없어도(레거시·EXIF없음) 캡션 UI 는 그리고,
...
/// 워터마크에 표시할 시각: 찍힌 시점(capture)만. 없으면 null → "No time".
/// 업로드 시각(received)으로 폴백하지 않는다 — 촬영 검증에 업로드 시각은 무의미.
/// (receivedAt 은 호출부 호환 위해 시그니처만 유지, 표시엔 쓰지 않음.)
