/// Stock Out screen — multi-item bulk stock out
///
/// Same structure as StockInScreen with these differences:
///   - Orange warning per item if qty would go negative or below min
///   - Save button in warning (orange) color
///   - Global warning banner when any item has issues
///   - Negative quantities are ALLOWED (warnings only, never blocking)
...
class _LineItem {
  ...
  bool get willBeNegative =>
      inventoryItem.currentQuantity - eaQuantity < 0;

  bool get willBeBelowMin =>
      inventoryItem.currentQuantity - eaQuantity <
...
class StockOutScreen extends ConsumerStatefulWidget {
  ...
  const StockOutScreen({super.key, required this.storeId});
...
class _StockOutScreenState extends ConsumerState<StockOutScreen> {
  ...
  bool get _hasWarnings =>
      _items.any((i) => i.willBeNegative || i.willBeBelowMin);
  ...
  Widget _buildWarningBanner() {
    final negativeCount = _items.where((i) => i.willBeNegative).length;
    final belowMinCount = _items
        .where((i) => !i.willBeNegative && i.willBeBelowMin)
        .length;
    final parts = <String>[];
    if (negativeCount > 0) {
      parts.add('$negativeCount item${negativeCount > 1 ? 's' : ''} will go negative');
    }
    if (belowMinCount > 0) {
      parts.add('$belowMinCount item${belowMinCount > 1 ? 's' : ''} below minimum');
    }
    return Container(
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Icon(Icons.add, size: 20, color: AppColors.warning),
                ),
              ),
            ],
          ),
              ...
              return Padding(
                ...
                child: _ItemRow(
                  ...
                  onUnitToggle: () => setState(
                      () => _items[i].useSubUnit = !_items[i].useSubUnit),
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: enabled ? AppColors.warning : null,
  ...
  Future<void> _save() async {
    ...
    final ok = await ref.read(inventoryProvider.notifier).bulkStockOut(
          widget.storeId,
          stockItems,
...
class _ItemRowState extends State<_ItemRow> {
  ...
  Widget build(BuildContext context) {
    ...
    final hasWarning = widget.lineItem.willBeNegative || widget.lineItem.willBeBelowMin;
