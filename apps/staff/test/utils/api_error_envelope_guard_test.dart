/// 강제장치 G5-b — **에러 봉투를 직접 파헤치지 않는다** (staff app).
///
...
/// 새로 만들어도 통과한다.
///
/// HTMA 에도 같은 테스트가 있다(`apps/attendance/test/unit/api_error_envelope_guard_test.dart`).
...
const Map<String, ({int count, String why})> _allowed = {
  'lib/providers/auth_provider.dart': (
    count: 5,
    why: '로그인/토큰 갱신 실패 detail 파싱(2개 헬퍼). 봉투 이전 코드 — extractApiError 로 이관 대기',
  ),
  'lib/screens/auth/register_screen.dart': (
    count: 3,
    why: '가입 실패 detail 파싱. 봉투 이전 코드 — 이관 대기',
  ),
  'lib/screens/auth/email_verification_screen.dart': (
    count: 3,
    why: '이메일 인증 실패 detail 파싱. 봉투 이전 코드 — 이관 대기',
  ),
  'lib/screens/auth/find_username_screen.dart': (
    count: 3,
    why: '아이디 찾기 실패 detail 파싱. 봉투 이전 코드 — 이관 대기',
  ),
  'lib/screens/auth/reset_password_screen.dart': (
    count: 3,
    why: '비밀번호 재설정 실패 detail 파싱. 봉투 이전 코드 — 이관 대기',
  ),
  'lib/services/clockin_pin_service.dart': (
    count: 3,
...
apps/staff/test/utils/api_error_envelope_guard_test.dart 의 _allowed 에
